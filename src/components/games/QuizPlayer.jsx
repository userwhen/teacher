...
export default function QuizPlayer({ activity, onFinish, onRestart }) {
  ...
  function handleConfirm() {
    ...
    setConfirmed(true)
    if (selected === item.answerIndex) setScore(s => s + 1)
    else setWrongCount(w => w + 1)
  }

  function handleNext() {
    if (current + 1 >= items.length) { if (onFinish) onFinish(score + (selected === item.answerIndex ? 1 : 0), items.length); setFinished(true) }
    ...
    return (
      <div className="card" style={{ textAlign:'center', padding:'2rem' }}>
        ...
        <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginBottom:'1.25rem' }}>
          ...
          <div style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', padding:'1rem', borderRadius:'var(--radius-md)', background:'var(--c-danger-bg)' }}>
  ...
  return (
    <div>
      ...
      <div className="card" style={{ marginBottom:'0.75rem' }}>
        <p style={{ fontSize:17, fontWeight:500, lineHeight:1.5 }}>{item.question}</p>
      </div>
      {item.options.map((opt, optIdx) => {
        ...
        return (
          <div key={optIdx} onClick={() => { if (!confirmed) setSelected(optIdx) }}
      ...
      <div style={{ marginTop:'1rem' }}>
        {!confirmed
          ? <button className="btn-primary" onClick={handleConfirm} disabled={selected===null} style={{ width:'100%', padding:12 }}>確認答案</button>
